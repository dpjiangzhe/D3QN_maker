...
class Actor(nn.Module):
    ...
    def forward(self, state):
        _a = torch.relu(self.layer_1(state))
        _a = torch.relu(self.layer_2(_a))
        _a = torch.tanh(self.output(_a))
        # 对action进行放缩，实际上a in [-1,1]
        scaled_a = _a * self.action_bound
        # print("actor forward:", _a, scaled_a, state)
        return scaled_a
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, action_bound, action_edge, replacement, memory_capacity=1000,
    ...
    def remember(self, s, a, r, s_, done=False):
        transition = np.hstack((s, a, [r], s_, done))
        index = self.pointer % self.memory_capacity
        self.memory[index, :] = transition
        self.pointer += 1

    def save_models(self, fpath="./", episode=0, fna="actor.pth", fnc="critic.pth"):
        fp = fpath + fna
        torch.save(self.actor.state_dict(), fp)
        fp = fpath + fnc
        torch.save(self.critic.state_dict(), fp)
        pass

    def load_models(self, fpath="./", episode=0, fna="actor.pth", fnc="critic.pth"):
        fp = fpath + fna
        self.actor.load_state_dict(torch.load(fp))
        fp = fpath + fnc
        self.critic.load_state_dict(torch.load(fp))
        pass
